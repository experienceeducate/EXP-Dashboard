...
- Second, third and fourth data sources added (ADR-008 + follow-up): mentor
  observation/roster data from `silver_exp.exp_2026_lec_observation_form`,
  `silver_exp.exp_2026_skills_day_observation_form`,
  `silver_exp.exp_2026_group_mentoring__observation__form`, and
  `bronze_exp.mentor_2026`, served by `app/routers/mentor_quality.py`
  (Mentor Quality tab's Highlights / LEC Observation / Group Mentoring /
  Skills Day sub-tabs). None are part of the gold model — CU-name spellings
  and term format differ from `gold_exp`, handled via
  `access_clause_fuzzy_cu`/`cu_clause_fuzzy` (`core/sql.py`). These are also
  live, evolving survey exports — column existence and even column *type* can
  change between requests (`database.get_table_columns()` guards against
  this). See ADR-008 for the full list of quirks before touching that router.
- Backend: implemented and unit-tested (auth, access scoping, client-header
  ...
  progress.
- Learning & Measurement Map tab added (ADR-009): static OKR/Learning Agenda
  reference content, not a BigQuery source — `frontend/src/data/learningMeasurementMap.js`,
  regenerated by hand from the source spreadsheet, see
  `docs/LEARNING_AND_MEASUREMENT_MAP.md`.
- Chat widget: intentionally not included (ADR-003) — superseded by E!nsight
  below, a different shape of feature (dashboard-first, generated SQL only as
  a guardrailed fallback), so ADR-003's reasoning doesn't apply to it.
- E!nsight added (ADR-010): dashboard-first Q&A over EXP programme data —
  `app/routers/ensight.py` + `core/ensight_catalog.py`/`ensight_calling.py`/
  `ensight_guardrails.py`/`ensight_llm.py`/`ensight_pipeline.py`,
  `frontend/src/views/EnsightView.jsx`. Gated to emails explicitly listed in
  `ACCESS_CONFIG["national"]` only (excludes `national_only` and all
  regional/CU staff) and to `OPENROUTER_API_KEY` being set. Generated SQL is
  the one documented exception to the never-f-string-SQL rule — see
  `core/ensight_guardrails.py` and ADR-010 for the substitute controls. Needs
  a 4th write-capable table, `raw_ensight_audit_log`
  (`DATA_ENG_BIGQUERY_TABLES_REQUEST.md` table 4) — degrades to a logged
  warning until that table exists, same pattern as the digest snapshot table.
  See `docs/ENSIGHT.md` for the full design.
...
- **`current_user()` must reconstruct every `UserAccess` field from the JWT,
  including `national_only`.** Found while building E!nsight (ADR-010): it
  was rebuilding `has_national`/`regions`/`cus` but silently dropping
  `national_only`, so it always came back `False` after token decode — the
  bug was invisible before ADR-010 because nothing server-side branched on it
  (the frontend's `nationalOnly` check only ever ran against the fresh
  `resolve_access()` result from `/api/auth/login`, not the JWT round-trip).
